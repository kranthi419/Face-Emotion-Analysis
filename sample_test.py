    ...
    for (x, y, w, h) in faces:
        ...
        if np.sum([roi_gray]) != 0:
            ...
            for i, (emotion, score) in enumerate(zip(emotion_labels, prediction)):
                # label_position = (x, y + (i * 20))
                # cv2.putText(frame, f"{emotion}: {score:.2f}", label_position, cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                #             (0, 255, 0), 1)
                label_position = (x + w + 10, y + (i * 20))
                color = (0, 255, 0) if score == max(prediction) else (0, 255, 255)
                cv2.putText(frame, f"{emotion}: {score:.2f}", label_position, cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 1)
        else:
            cv2.putText(frame, 'No Faces', (30, 80), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
